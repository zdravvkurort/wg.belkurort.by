...
function create_contact($name, $phone, $email, $responsible_id, $utm_data=[])
	{
		global $amo;
		$contact = $amo->contact;
		// $contact->debug(true); 
	...
	function create_lead($phone, $responsible_id, $pipeline_id, $utm_data, $date_create)
	{
		global $amo;
		$lead = $amo->lead;
	    $lead['name'] = 'Заказ с сайта '.$utm_data['site'];
	    ...
	    $lead['tags'] = [$utm_data['site'], 'Venyoo'];
		
		//$lead->addCustomField(305083, 519283);
		
	    if(isset($utm_data['site']) and $utm_data['site'] != "") {
			$lead->addCustomField(352877, $utm_data['site']);
			};	
	    if(isset($utm_data['utm_source']) and $utm_data['utm_source'] != "") {
			$lead->addCustomField(306047, $utm_data['utm_source']);
			};
	    if(isset($utm_data['utm_campaign']) and strlen($utm_data['utm_campaign']) != 0) {
			$lead->addCustomField(306049, $utm_data['utm_campaign']);};
	    if(isset($utm_data['utm_term']) and strlen($utm_data['utm_term']) != 0) {
			$lead->addCustomField(306051, $utm_data['utm_term']);
		};
	    if(isset($utm_data['utm_medium']) and strlen($utm_data['utm_medium']) != 0) {
			$lead->addCustomField(306053, $utm_data['utm_medium']);
			};
	    if(isset($utm_data['utm_content']) and strlen($utm_data['utm_content']) != 0) {
			$lead->addCustomField(306055, $utm_data['utm_content']);
			};
		if(isset($utm_data['clientid_ga']) and strlen($utm_data['clientid_ga']) != 0) {
			$lead->addCustomField(311597, $utm_data['clientid_ga']);
			};
...
//			        'query' => substr($p,$tfr,iconv_strlen($phone)),
					'query' => substr($p,0,iconv_strlen($phone)),
			 ]);
