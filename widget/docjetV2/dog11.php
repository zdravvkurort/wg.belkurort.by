...
$saveResult = saveFileByLinkOnYaDisk( $pdfUrl, 
																			'leads', 
																			$card_id, 
																			'Договор '.explode(" ",$fio[0])[0].' '.$time.' '.$date.'.pdf', 
																			'/Генерация');

if(!$saveResult) {
	create_task($card_id, 'Договор после генерации не сохранён. Номер договора: '.$data['nomer_dogovora'], 3406348);
}
/*
