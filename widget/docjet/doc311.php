...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
  ...
  try {
    ...
    $pdf->AddPage();
	$pdf->Image("stamp_2.png", 55, 185, 55, 40);	
...
<style>
        * {
        ...
        td.signature, tr.signature, table.signature{
            border: none;
			text-align: left;
        }
        h4 {
...
УНП 193237911<br/>
====================================================================</p>
<p class = "center">Санаторно-курортная путевка №'.$nomer_dogovora.'</p>
		<table>
            <tr>
              <td>Объект отдыха</td>
              <td>'.$dog_naimenovanie_obekta_razmescheniya.'</td>
            </tr>
            <tr>
              <td>Даты заезда/выезда</td>
              <td>c '.date("d.m.Y",strtotime($data_zaezda)).' по '.date("d.m.Y",strtotime($data_vyezda)).'</td>
            </tr>
            <tr>
              <td>Адрес объекта</td>
              <td>'.$dog_adres_obekta_razmescheniya.'</td>
            </tr>
            <tr>
              <td>Данные отдыхающих по путевке:</td>
              <td>'.$dog_edet_li_turist_dogovor.'<br/>
'.$turist_2.'<br/>
'.$turist_3.'<br/>
'.$turist_4.'<br/>
'.$turist_5.'</td>
            ...
            <tr>
              <td>В стоимость путевки входит:</td>
              <td>1. Питание, заказное меню с 3го дня, 6 разовое<br/>
2. Проживание<br/>
3. '.$dog_lechenie.'</td>
            </tr>
            <tr>
            ...
            <tr>
              ...
              <td>'.$kolichestvo_nomerov.'</td>
            </tr>
        </table>
<p>Внимание! При заселении оплачивается санаторно-курортный сбор (местный налог)</p>
<p></p>
<p></p>
<p></p>
<table class="signature">
<tr class="signature">
<td width="50%" class="signature">Специалист по туризму</td>
<td width="50%" class="signature">'.$manager_insert.'</td>
</tr>
</table>
</body>
</html>';
...
$time = date('H:i:s');	
    //$pdf->Output('form.pdf');
	$pdf->Output($card_id.'/'.$date.' '.$time.' Санаторно-курортная путевка + '.$fio[0].'.pdf');
