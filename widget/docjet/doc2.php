...
$document ->save($card_id.'/'.$date.' '.$time.' Заявка на бронирование + '.$fio[0].'.docx');
		...
		header('Content-Disposition: attachment; filename=Заявка на бронирование + '.$fio[0].'.docx');
        header('Content-Transfer-Encoding: binary');
        header('Expires: 0');
        header('Cache-Control: must-revalidate');
        header('Pragma: public');
		header('Content-Length: ' . filesize($card_id.'/'.$date.' '.$time.' Заявка на бронирование + '.$fio[0].'.docx'));
        flush();
		readfile($card_id.'/'.$date.' '.$time.' Заявка на бронирование + '.$fio[0].'.docx');
...
$notes['request']['notes']['add']=array(
 array(
    ...
    'element_type'=>2,
    'note_type'=>4,
    'text'=>'Сформирована Заявка на бронирование на скачивание: http://wg.belkurort.by/widget/docjet/'.$card_id.'/'.$date.'%20'.$time.'%20Заявка%20на%20бронирование%20+%20'.$fio_insert.'.docx',
  ),
);
