...
$PHPWord = new PHPWord();
$document = $PHPWord->loadTemplate('templates/booking_template.docx'); //шаблон
...
$date = date('d-m-Y');
$time = date('H:i:s');
$document ->save($card_id.'/'.$date.' '.$time.' Заявка на бронирование '.$fio[0].'.docx');
...
$file1 = $myTaskConvertOffice->addFile($card_id.'/'.$date.' '.$time.' Заявка на бронирование '.$fio[0].'.docx');
// Execute the task
...
header("Content-Type: text/html; charset=utf-8");
		...
		header('Content-Disposition: attachment; filename='.$card_id.'/'.$date.' '.$time.' Заявка на бронирование '.$fio[0].'.pdf');
        header('Content-Transfer-Encoding: binary');
        header('Expires: 0');
        header('Cache-Control: must-revalidate');
        header('Pragma: public');
		header('Content-Length: ' . filesize($card_id.'/'.$date.' '.$time.' Заявка на бронирование '.$fio[0].'.pdf'));
        flush();
		readfile($card_id.'/'.$date.' '.$time.' Заявка на бронирование '.$fio[0].'.pdf');
...
$notes['request']['notes']['add']=array(
 array(
    ...
    'element_type'=>2,
    'note_type'=>4,
    'text'=>'Сформирована Заявка на бронирование на скачивание: http://wg.belkurort.by/widget/docjet/'.$card_id.'/'.$date.'%20'.$time.'%20Заявка%20на%20бронирование%20'.$fio_insert.'.pdf',
  ),
);
...
unlink($card_id.'/'.$date.' '.$time.' Заявка на бронирование '.$fio[0].'.docx');