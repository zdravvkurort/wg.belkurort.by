...
if(isset($_GET['startdate']) and isset($_GET['enddate']) and isset($_GET['offset'])) {
  ...
  $table = array_merge($table, makeTable($managerStore, $offset));
...
function makeSector($manager, $offset) {
  $resultArray = [];
  array_push($resultArray, makeRow($manager, "start_working", $offset));
  array_push($resultArray, makeRow($manager, "end_working", $offset));
  array_push($resultArray, makeRow($manager, "rests", $offset));
  array_push($resultArray, makeRow($manager, "online_duration", $offset));
  return $resultArray;
}
...
function makeRow($manager, $indicator, $offset = 0) {
  ...
  $row = ($indicator == "start_working") ? [$manager["name"]] : [""];
...
function getStartEndWorking($managerStore, $datesArray) {
  ...
  foreach($datesArray as $key => $date) {
    $stmt = $db->query("SELECT users.id, users.name, MIN(notes_all.`created_at`) as start_working, MAX(notes_all.`created_at`) as end_working
                        ...
                        WHERE notes_all.`created_at` >= ".$date["start"]." and notes_all.`created_at` <= ".$date["end"]." and users.group_id = 0 and users.active = 1
                        GROUP BY users.id");
